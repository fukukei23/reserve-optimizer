...
- 個人開発であるため、「AI活用型開発」を前提に生産性を最大化する。

## 7. 販売戦略

### 価格・提供形態
- **月額価格**: 検討中（マーケットリサーチと実証データを基に決定）
- **提供形態**: 検討中（マルチテナントSaaS型 or 個別構築型）
- **集金システム**: Stripe（サブスクリプション決済）

### 営業チャネル戦略
- **Phase 1**: 整骨院コミュニティへの参加・貢献と、X（旧Twitter）での発信により、月1〜2件の初期契約を目標とする。
...
- **最終的な成功**: ドメイン知識を習得し、保守・運用が回る自立したSaaSプロダクトとして成立していること。ポートフォリオとして独立・受託・キャリア構築に活用できる状態であること。
