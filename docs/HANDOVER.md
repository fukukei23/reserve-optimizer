...
> ⚠️ **2026-08-04 注記**: 本資料は一部情報が古くなっています。「5. 未実装・準備中」と記載の **予約変更 / キャンセル / 待機リスト登録 は実装済み**です（`handlers/ChangeHandler`・`CancelHandler`・`WaitlistHandler`・ステートマシン15状態に統合）。最新の機能一覧は [README.md](../README.md) / [PROJECT_ANALYSIS.md](PROJECT_ANALYSIS.md) を参照してください。
