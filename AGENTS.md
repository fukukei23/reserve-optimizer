...
## 💰 コスト表示ルール（2026-03-19追加）

**全ての応答の末尾に、以下の形式でコスト試算を記載する:**
...
**計算方法:**
```
コスト = (入力トークン × $1/千トークン + 出力トークン × $3.2/千トークン) × 汇率160
```
...
**【2026-03-19 コスト計算式 誤記修正】**
- 誤: コスト = (入力トークン × $1/千トークン + 出力トークン × $3.2/千トークン) × 汇率160
- 正: コスト = (入力トークン ÷ 1,000,000 × $1) + (出力トークン ÷ 1,000,000 × $3.2)
- 例: 入力300 + 出力400トークン → $0.00158（約0.25円）

---

## 🧮 計算ルール（2026-03-20追加）

**原則**: 算数・計算はLLMの苦手分野。execに外包する。
...
**例**:
```bash
# 50token × 1440回 ÷ 1,000,000 × $1
python3 -c "print(50 * 1440 / 1000000 * 1)"
# → 0.072
```

**例外**: 3 + 5 程度の単純な足し算は暗算OK
